<div class="page-heading">
    <div class="container">
        <div class="row">
            <div class="col-md-12">
                <div class="page-heading-content">
                    <span><a href="index.php?page=home">Home</a>/<em>Blogs</em></span>
                    <h1>Blogs</h1>
                </div>
            </div>
        </div>
    </div>
</div>
<div class="blog-content">
    <div class="container">
        <div class="row">
            <div class="col-md-12">
                <div class="blog-posts">
                    <div class="row">
                        <div class="col-md-6">
                            <div class="blog-post">
                                <div class="blog-thumb">
                                    <img src="assets/images/blog1.jpg" alt="">
                                    <span>Family and Friend</span>
                                </div>
                                <h4>How to explain autism to your family and friends</h4>
                                <ul>
                                    <li><a href="#">Admin</a></li>
                                    <li><a href="#">July 10, 2020</a></li>
                                </ul>
                                <p> Awareness of autism is growing, with greater representation in our culture thanks to public figures like Chris Packham, Dan Ackroyd and Susan Boyle speaking about their own experiences</p>
                                <a href="index.php?page=family-and-friend" class="read-more">Read More</a>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="blog-post">
                                <div class="blog-thumb">
                                    <img src="assets/images/blog2.jpg" alt="">
                                    <span>Child Refuses</span>
                                </div>
                                <h4>What to do when your child refuses to go to school</h4>
                                <ul>
                                    <li><a href="#">Admin</a></li>
                                    <li><a href="#">Mar 15, 2020</a></li>
                                </ul>
                                <p>When children completely refuse to go to school this can be extremely distressing for all parties.  It is often accompanied by melt-downs, physical complaints such as stomach aches, headaches, vomiting, rapid heartbeat, dizziness, pleading or begging to stay at home and even young people threatening to harm themselves if they are made to go to school.</p>
                                <a href="index.php?page=child-refuses" class="read-more">Read More</a>
                            </div>
                        </div>
                        <div class="col-md-12">
                            <ul class="blog-pagination">
                                <li><a href="#"><i class="fa fa-angle-double-left"></i></a></li>
                                <li class="active"><a href="#">1</a></li>
                                <li><a href="#"><i class="fa fa-angle-double-right"></i></a></li>
                            </ul>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>